...
HS_CONSTANT_DATA_OUTPUT PassThroughConstantHS(InputPatch<HS_IO, 3> ip, unit PatchID : SV_PrimitiveID)
{
...
//Domain Shader
[domain("tri")]
PS_INPUT DSMAIN(HS_CONSTANT_DATA_OUTPUT input, float3 BarycentricCorrdinates : SV_DomainLocation, const OutputPatch<HS_IO, 3> TrianglePatch)
{
	PS_INPUT output;
	...
	float3 vWorldPos = BarycentricCoordinates.x * TrianglePatch[0].Pos
		...
		+ BarcentricCoordinates.z * TrianlgePatch[2].Pos;
